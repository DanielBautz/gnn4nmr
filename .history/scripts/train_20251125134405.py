...
def train_model(model, train_loader, val_loader, test_loader, device, config):
    ...
    pickle.dump(original_dataset.norm_stats, open('norm_stats.pkl', 'wb'))
    edge_stats = {
    ...
    pickle.dump(edge_stats, open('edge_stats.pkl', 'wb'))
    ...
    for epoch in range(config.num_epochs):
        ...
        if val_score <= best_val_score:
            ...
            torch.save(model.state_dict(), f"{config.operator_type}_best_model.pt")
            early_stopping_counter = 0
        else:
