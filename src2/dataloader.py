...
def create_dataloaders(batch_size=4, root_dir="data", file_name="all_graphs.pkl", 
                       split_ratio=(0.8, 0.1, 0.1), k_folds=1, fold_idx=0,
                       normalize_node_features=True, normalize_edge_features=True,
                       random_seed=0):
    """
    Erstellt Dataloaders mit Unterstützung für k-fold Cross-Validation.
    
    Parameter:
    - batch_size: Batch-Größe für die Dataloaders
    - root_dir: Verzeichnis mit den Daten
    - file_name: Name der Pickle-Datei mit Graphdaten
    - split_ratio: Tupel aus (train, val, test) Verhältnissen
    - k_folds: Anzahl der Folds für Cross-Validation (Standard: 1 = Standard-Split)
    - fold_idx: Index des aktuellen Folds (0-basiert, Standard: 0)
    - normalize_node_features: Ob Knoten-Features normalisiert werden sollen
    - normalize_edge_features: Ob Kanten-Features normalisiert werden sollen
    - random_seed: Seed für die Zufallszahlengenerierung (Standard: 0)
    ...
    if k_folds == 1:
        ...
        fold_size = num_compounds // k_folds
        test_compounds = []
        val_compounds = []
        train_compounds = []
        
        # Teile die Compounds in k_folds ungefähr gleich große Teile
        fold_compounds = []
        for i in range(k_folds):
            start_idx = i * fold_size
            end_idx = num_compounds if i == k_folds - 1 else (i + 1) * fold_size
            fold_compounds.append(compounds[start_idx:end_idx])
        
        # Für den aktuellen fold_idx: Nimm diesen als Test, den nächsten (oder ersten) als Validation
        # und alle anderen als Training
        test_compounds = fold_compounds[fold_idx]
        val_idx = (fold_idx + 1) % k_folds
        val_compounds = fold_compounds[val_idx]
        
        for i in range(k_folds):
            if i != fold_idx and i != val_idx:
                train_compounds.extend(fold_compounds[i])
    
    # Konvertiere Compounds zu Indices
    train_indices = []
