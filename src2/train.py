...
def train_model(model, train_loader, val_loader, test_loader, device, config, fold_idx=None):
    """
    Haupttrainingsschleife:
      - Single-Model, multi-task (H und C).
      - Loggt train/val/test MSE und MAE für beide Targets.
      - Unterstützt k-fold Validierung über fold_idx.
    """
    ...
    model_save_path = f"best_model_fold_{fold_idx}.pt" if fold_idx is not None else "best_model.pt"
    ...
    for epoch in range(config.num_epochs):
        ...
        # Add fold information to logs if using k-fold
        if fold_idx is not None:
            log_data["fold"] = fold_idx
        
        wandb.log(log_data)
        
        # Add fold prefix to logs if using k-fold
        fold_prefix = f"Fold [{fold_idx+1}/{config.k_folds}] " if fold_idx is not None else ""
        print(f"{fold_prefix}Epoch [{epoch+1}/{config.num_epochs}]")
    ...
    fold_prefix = f"Fold [{fold_idx+1}/{config.k_folds}] " if fold_idx is not None else ""
    print(f"{fold_prefix}** Test ** => H: MSE={test_mse_H:.4f}, MAE={test_mae_H:.4f} |"
          f"  C: MSE={test_mse_C:.4f}, MAE={test_mae_C:.4f}")
    
    return model, best_val_score, test_score